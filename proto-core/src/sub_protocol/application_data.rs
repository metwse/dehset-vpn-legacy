...
pub enum ApplicationData {
    ...
    /// Message sent by the server to the client to initiate a tunneled connection.
    /// Connection ID generated by the server, which may differ from
    /// the client's `connection_id` in [`ApplicationData::RequestConnection`].
    NewConnection { port: u16 },

    /// Response from the port-sharing client to a
    /// [`ApplicationData::NewConnection`] payload. This message is forwarded
    /// by the server to the port-requesting client.
...
#[derive(Debug, Serialize, Deserialize)]
pub struct ApplicationDataPayload {
    pub connection_id: u64,
    pub payload: ApplicationData,
}
