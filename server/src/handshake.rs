...
pub(crate) async fn handshake(
    (tcp_stream, remote_addr): (&mut TcpStream, SocketAddr),
    state: Arc<SharedState>,
) -> Result<(), Error> {
    info!("Awaiting handshake...");
    let payload = read_payload(tcp_stream).await?;
    ...
    {
        return Err(Error::Handshake("invalid payload or content type"));
    }
    ...
    let authenticate = read_payload(tcp_stream).await?;
    ...
    let _server_random = authenticate.random;
    if !state.signer.verify(
        &authenticate.token.token.encode()?,
        &authenticate.token.signature,
    )? {
        return Err(Error::Handshake("could not verify token"));
    }
    info!("Authenticated {:?}", authenticate);
